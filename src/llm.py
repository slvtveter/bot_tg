...
_FORMATTING_PHILOSOPHY = (
    ...
    "разделитель '|---|---|'.\n"
    "- Формулы и математику оформляй единообразно: оборачивай их в $...$ (в "
    "строке) или $$...$$ (отдельным блоком) — современный Telegram рендерит "
    "LaTeX. НИКОГДА не оставляй команды LaTeX (\\frac, \\nabla, \\sum, "
    "\\partial, \\left) без $-обёртки и не используй окружения "
    "\\begin{...}/\\end{...} — без обёртки они покажутся пользователю как сырой "
    "текст. Совсем простое можно дать и Unicode-символами (∇f, x², ∂f/∂x), но "
    "не смешивай голый LaTeX с обычным текстом."
)
