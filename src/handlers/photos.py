...
async def photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if mode == "math":
            ...
            vision_prompt = f"{base} Запрос пользователя: {caption}" if caption else base
        else:  # general: treat the image as context; nutrition is opt-in
            nutrition_requested = mode == "nutrition" or any(
                word in caption.lower()
                for word in ("кбжу", "калори", "белк", "жир", "углевод", "питани", "нутрици")
            )
            if nutrition_requested:
                vision_prompt = (
                    "Если это еда, оцени КБЖУ и в конце добавь строку "
                    "[NUTRITION_DATA] calories=ЧИСЛО protein=ЧИСЛО fat=ЧИСЛО carbs=ЧИСЛО. "
                    "Если это не еда, ответь по запросу без этой строки."
                )
            else:
                vision_prompt = (
                    "Проанализируй изображение как контекст для запроса пользователя. "
                    "Ответь непосредственно на вопрос или просьбу по изображению. "
                    "Не предполагай, что это еда, не анализируй КБЖУ и не добавляй "
                    "[NUTRITION_DATA], если пользователь явно не просит об этом."
                )
            if caption:
                vision_prompt += f"\n\nЗапрос пользователя к этому изображению: {caption}"
